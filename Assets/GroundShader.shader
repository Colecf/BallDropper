...
#define _Color_arr Props
		...
		float3 p = float3(_Time.y * 5, i.worldPos.x, i.worldPos.z) / 10;
		//float scale = length(float2(mod(i.worldPos.x, 1.0), mod(i.worldPos.z, 1.0)))/10;
		float scale = 10.0f;
		float x = abs(0.5 - abs(mod(i.worldPos.x + pnoise(p, float3(1.5, 1.5, 1.5)) * 2 * 1, scale))/scale)*2.0;
		float z = abs(0.5 - abs(mod(i.worldPos.z + pnoise(p, float3(1.5, 1.5, 1.5)) * 2 * 1, scale))/scale)*2.0;
			float distToEdge = min(x, z)*4;
			distToEdge -= pnoise(p, float3(1.5, 1.5, 1.5)) * 0.5;
